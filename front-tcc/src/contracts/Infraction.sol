...
contract InfractionTcc{
    ...
    struct TraficTicket{
        ...
        string observations;
        uint valueToPay; // Value in reais
        uint statusOfInfraction; // Active, Payed, Canceled, Transfered
    ...
    mapping(address => mapping(uint256=>TraficTicket)) public driveTickets; // we shall have the tickets mapped against each address to recover all drivers tickets
    mapping(address => uint256) individualCountOfTraficTicket;			// how many tickets does a particular person hold

    
    mapping(uint => Drivers) public drivers; // Para simular o banco auxiliar de motoristas
    mapping(address => Drivers) public driversListAddress;
    ...
    function registerInfraction(string memory _vehiclePlate, uint _infractionCategory,
        ...
        uint driverPointsAfterInfraction = driversListAddress[_infractorDriverAddress].numberOfPoints + _infractionPoints;
